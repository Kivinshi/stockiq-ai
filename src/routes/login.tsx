...
export const Route = createFileRoute("/login")({
  head: () => ({ meta: [{ title: "Log in — StockIQ" }, { name: "description", content: "Log in to your StockIQ workspace." }] }),
  component: Login,
});
...
function Login() {
  ...
  function submit(e: React.FormEvent) {
    ...
    setLoading(true);
    setTimeout(() => {
      setSession({ name: email.split("@")[0].replace(/\./g, " "), email, role });
    ...
    >
      <form onSubmit={submit} className="space-y-4">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground mb-2">Select a role to preview the dashboard (no backend).</p>
          <div className="grid grid-cols-4 gap-2">
            {(["admin","manager","staff","viewer"] as Role[]).map(r => (
              <button key={r} type="button" onClick={() => setRole(r)}
                className={`text-xs font-medium py-2 rounded-lg border transition ${role===r ? "bg-gradient-primary text-primary-foreground border-transparent shadow-elegant" : "border-border hover:bg-accent"}`}>
                {ROLE_META[r].label}
              </button>
            ))}
          </div>
...
function Field({ label, icon: Icon, right, children }: { label: string; icon: any; right?: React.ReactNode; children: React.ReactNode }) {
  return (
    <div>
