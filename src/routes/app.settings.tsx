        ...
        <Card title="Appearance">
          ...
          <div className="grid grid-cols-2 gap-3">
            {(["light","dark"] as const).map(t => (
              <button key={t} onClick={()=>{setTheme(t); toast.success(`Theme set to ${t}`);}}
                className={`p-4 rounded-xl border-2 transition ${theme===t?"border-primary shadow-elegant":"border-border hover:bg-accent"}`}>
                <div className={`h-16 rounded-lg ${t==="light"?"bg-white border border-border":"bg-foreground"}`} />
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div>
      ...
      <input defaultValue={value} className="mt-1 w-full h-10 px-3 rounded-lg border border-input bg-background text-sm outline-none focus:border-primary focus:ring-2 focus:ring-primary/20 transition" />
    </div>
  );
}
...
function Toggle({ label, desc, defaultOn = false }: { label: string; desc: string; defaultOn?: boolean }) {
  return (
    <label className="flex items-center justify-between py-3 border-b border-border last:border-0 cursor-pointer">
      ...
      <span className="relative h-6 w-11 rounded-full bg-muted peer-checked:bg-gradient-primary transition shrink-0 ml-3
