...
import {
  LayoutDashboard, Boxes, Package, ShoppingCart, Truck, FileBarChart2, Users2,
  Bell, Settings, User, Moon, Sun, LogOut, Search, ChevronLeft, Menu, X, Sparkles
} from "lucide-react";
...
const NAV: NavItem[] = [
  ...
  { to: "/app/products",      label: "Products",   icon: Package,        roles: ["admin","manager","staff","viewer"] },
  { to: "/app/inventory",     label: "Inventory",  icon: Boxes,          roles: ["admin","manager","staff","viewer"] },
  { to: "/app/sales",         label: "Sales",      icon: ShoppingCart,   roles: ["admin","manager","staff","viewer"] },
  { to: "/app/suppliers",     label: "Suppliers",  icon: Truck,          roles: ["admin","manager"] },
  { to: "/app/reports",       label: "Reports",    icon: FileBarChart2,  roles: ["admin","manager","viewer"] },
  { to: "/app/users",         label: "Users",      icon: Users2,         roles: ["admin"] },
  { to: "/app/notifications", label: "Notifications", icon: Bell,        roles: ["admin","manager","staff","viewer"] },
...
export function AppShell({ children, title, subtitle }: { children: ReactNode; title: string; subtitle?: string }) {
  ...
  return (
    <div className="min-h-screen flex bg-muted/30">
      {/* Sidebar */}
      <aside className={`hidden md:flex flex-col bg-sidebar border-r border-sidebar-border transition-all duration-300 ${collapsed ? "w-16" : "w-64"}`}>
        <div className="h-16 flex items-center justify-between px-4 border-b border-sidebar-border">
          <Link to="/" className="flex items-center gap-2 font-display font-bold overflow-hidden">
            <span className="grid place-items-center h-9 w-9 rounded-xl bg-gradient-primary shrink-0"><Boxes className="h-5 w-5 text-primary-foreground" /></span>
            {!collapsed && <span className="truncate">Stock<span className="text-gradient">IQ</span></span>}
          </Link>
          <button onClick={() => setCollapsed(!collapsed)} className="h-8 w-8 grid place-items-center rounded-lg hover:bg-sidebar-accent transition shrink-0">
        ...
        <div className="p-3 border-t border-sidebar-border">
          {!collapsed && (
            <div className="mb-3 rounded-xl p-3 bg-gradient-primary text-primary-foreground text-xs">
              <div className="flex items-center gap-2 font-semibold"><Sparkles className="h-3 w-3" /> AI Assistant</div>
              <p className="mt-1 opacity-90">Ask: which product needs restock?</p>
            </div>
          )}
          <Link to="/app/profile" className="flex items-center gap-3 p-2 rounded-lg hover:bg-sidebar-accent transition">
      ...
      {mobile && (
        <div className="fixed inset-0 z-50 md:hidden">
          <div className="absolute inset-0 bg-background/80 backdrop-blur" onClick={() => setMobile(false)} />
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-16 flex items-center gap-3 px-6 bg-background/70 backdrop-blur border-b border-border sticky top-0 z-30">
          <button onClick={() => setMobile(true)} className="md:hidden h-9 w-9 grid place-items-center rounded-lg hover:bg-accent"><Menu className="h-4 w-4" /></button>
          <div className="hidden sm:flex items-center gap-2 h-9 px-3 rounded-lg bg-muted/60 border border-border max-w-md flex-1">
            ...
            <input placeholder="Search products, orders, users…" className="w-full bg-transparent outline-none text-sm" />
            <kbd className="text-[10px] px-1.5 py-0.5 rounded bg-background border border-border text-muted-foreground">⌘K</kbd>
          </div>
          <div className="ml-auto flex items-center gap-2">
            <span className={`hidden sm:inline-flex text-xs font-medium px-2.5 py-1 rounded-full ${meta.color}`}>{meta.label}</span>
            <button onClick={toggle} className="h-9 w-9 grid place-items-center rounded-lg hover:bg-accent" aria-label="Theme">
              {theme==="dark" ? <Sun className="h-4 w-4" /> : <Moon className="h-4 w-4" />}
            </button>
            <Link to="/app/notifications" className="relative h-9 w-9 grid place-items-center rounded-lg hover:bg-accent">
              <Bell className="h-4 w-4" />
              <span className="absolute top-1.5 right-1.5 h-2 w-2 rounded-full bg-destructive animate-pulse-glow" />
            </Link>
            <button onClick={logout} className="h-9 w-9 grid place-items-center rounded-lg hover:bg-accent text-muted-foreground" title="Sign out">
        ...
        <main className="flex-1 p-6 lg:p-8">
          ...
          <div className="animate-fade-up" style={{ animationDelay: "60ms" }}>{children}</div>
        </main>
      </div>
    </div>
  );
...
export function KpiCard({ label, value, hint, icon: Icon, trend, accent = "primary" }:
  { label: string; value: string; hint?: string; icon: any; trend?: string; accent?: "primary"|"success"|"warning"|"destructive" }) {
  const tone = { primary: "bg-primary/10 text-primary", success: "bg-success/15 text-success", warning: "bg-warning/20 text-warning-foreground", destructive: "bg-destructive/15 text-destructive" }[accent];
  return (
...
export function Badge({ children, tone = "muted" }: { children: ReactNode; tone?: "muted"|"success"|"warning"|"destructive"|"primary" }) {
  ...
  return <span className={`inline-flex items-center text-xs font-medium px-2 py-0.5 rounded-md ${tones[tone]}`}>{children}</span>;
}
